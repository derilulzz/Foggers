function createTipRect()
    local t = {
        ...
        tips = {
            ...
            "Use the numbers in your keyboard to quickly select cars",
            "Use the left Ctrl to speed up",
        },
        tipsPT = {
            "Aperte shift para rapidamente acelerar ou desacelerar o jogo",
            "Use os números no seu teclado para rapidamente selecionar carros",
            "Use o Ctrl esquerdo para acelerar",
        },
        alpha = 0.5,
    }
    ...
    function t:init()
        ...
        Flux.to(self.pos, 1, { x = 800 - self.size.w }):ease("expoout"):after(self, 1, { alpha = 0 }):delay(2):ease(
            "expoin"):oncomplete(t.destroy)
    end
    ...
    function t:draw()
        ...
        love.graphics.setColor({ 1, 1, 1, self.alpha * 2 })
        drawOutlinedTextF(self.text, self.pos.x, self.pos.y + 32 + 8, self.size.w / 2, "center", 0, 2, 2, 0, 0, 4,
            { 0, 0, 0, self.alpha })
    end
