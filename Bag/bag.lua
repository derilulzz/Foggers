...
function createMoneyPrinter()
    local m = {
        pos = { x = 0, y = 0 },
        spr = newAnimation(love.graphics.newImage("Sprs/Items/MoneyPrinter/PrinterSpr.png"), 32, 40, 28, 0, 14.5, 1),
        scale = {
            x = 4,
            y = 4
        },
    ...
    function m:draw()
        ...
        self.spr:draw(0, self.pos.x, self.pos.y, self.scale.x, self.scale.y + 0.1 * math.sin(GlobalSinAngle), nil, nil,
            nil, nil)
    end
...
function createCarCreator()
    local m = {
        pos = { x = 0, y = 0 },
        spr = newAnimation(love.graphics.newImage("Sprs/Items/MoneyPrinter/CarCreator.png"), 30, 22, 3, 0, 1.5, 1),
        scale = {
            x = 4,
            y = 4
        },
...
function createCarGenerationOrb(x, y, targetX, targetY)
    local o = {
        ...
        timeToReach = 1,
        spr = newAnimation(love.graphics.newImage("Sprs/Items/MoneyPrinter/CarCreateOrb.png"), 16, 16, 3, 0, 10, 0),
    }


    function o:init()
        Flux.to(self.pos, self.timeToReach, { x = self.targetPos.x, y = self.targetPos.y }):ease("expoin"):oncomplete(o
        .destroySelf)
    end
    ...
    if self.placingItem then
        ...
        for c = 1, #gameInstances do
            if gameInstances[c].col ~= nil then
                local realRect = createRectangle(PushsInGameMousePos.x - self.currentPlacingItem.col.w / 2,
                    PushsInGameMousePos.y - self.currentPlacingItem.col.h / 2, self.currentPlacingItem.col.w,
                    self.currentPlacingItem.col.h)
        ...
        if canUpdatePos then
            ...
            self.currentPlacingItem.col.x = self.currentPlacingItem.pos.x -
            (self.currentPlacingItem.spr.sprWidth * self.currentPlacingItem.scale.x) / 2
            self.currentPlacingItem.col.y = self.currentPlacingItem.pos.y -
            (self.currentPlacingItem.spr.sprHeight * self.currentPlacingItem.scale.y) / 2
            ...
            self.currentPlacingItem.col.x = PushsInGameMousePosNoTransform.x -
            (self.currentPlacingItem.spr.sprWidth * self.currentPlacingItem.scale.x) / 2
            self.currentPlacingItem.col.y = PushsInGameMousePosNoTransform.y -
            (self.currentPlacingItem.spr.sprHeight * self.currentPlacingItem.scale.y) / 2
            self.currentPlacingItem.col.w = self.currentPlacingItem.spr.sprWidth * self.currentPlacingItem.scale.x
            self.currentPlacingItem.col.h = self.currentPlacingItem.spr.sprHeight * self.currentPlacingItem.scale.y
        end
        ...
        if not love.mouse.isDown(1) then
            local newPos = { gameCam.transform:inverseTransformPoint(self.currentPlacingItem.pos.x,
                self.currentPlacingItem.pos.y) }
...
function bagStuff:draw()
    if self.showingBag then
        ...
        drawOutlinedText(text, self.bagRect.x + self.bagRect.w / 2, self.bagRect.y + 32 + 8 * math.sin(GlobalSinAngle), 0,
            4, 4, love.graphics.getFont():getWidth(text) / 2, love.graphics.getFont():getHeight(text) / 2, 4, { 0, 0, 0 })
        ...
        while i < #self.stored do
            ...
            if self.hoveredId == i + 1 and self.hoveringAnItem then
                color = { 1, 1, 1 }; scale = scale + self.currentSlotScaleAdd
            end
            ...
            if i - iDec == 1 then
                drawOutlinedText(itemName, x, y, 0, scale, scale, love.graphics.getFont():getWidth(itemName) / 2,
                    love.graphics.getFont():getHeight(itemName) / 2, 2, { 0, 0, 0 })
                previousWidth = (love.graphics.getFont():getWidth(itemName) * scale)
            else
                drawOutlinedText(itemName, x, y, 0, scale, scale, 0, love.graphics.getFont():getHeight(itemName) / 2, 2,
                    { 0, 0, 0 })
        ...
        if self.showDescTimer >= 1 and self.hoveringAnItem then
            ...
            if self.stored[self.hoveredId] ~= nil then
                ...
                drawOutlinedRect(PushsInGameMousePosNoTransform.x + 32 - xDec, PushsInGameMousePosNoTransform.y + 32,
                    limit * 2, ((love.graphics.getFont():getHeight() * #wrap[2]) * 2) + 4, { 1, 1, 1 })
                drawOutlinedTextF(text, PushsInGameMousePosNoTransform.x + 32 - xDec,
                    PushsInGameMousePosNoTransform.y + 32, limit, "center", 0, 2, 2, 0, 0, 2, { 0, 0, 0 })
            end
        end
        ...
        if #self.stored <= 0 then
            ...
            drawOutlinedText(text, self.bagRect.x + self.bagRect.w / 2, self.bagRect.y + self.bagRect.h / 2, 0,
                2 + 0.25 * math.cos(GlobalSinAngle), 2 + 0.25 * math.cos(GlobalSinAngle), nil, nil,
                4 + 0.25 * math.cos(GlobalSinAngle), { 0, 0, 0 })
        end
